...
void animate() {
  ...
  for(int i = 0; i < GRAPH_TRIS; i++) {
    ...
    vertexes[i].z += animCounter % 120 < 60 ? .1 : -.1;

    //vertexes[i].x *= animCounter % 122 < 62 ? 1.01 : (1 / 1.01);
    // vertexes[i].y *= animCounter % 122 < 62 ? 1.01 : (1 / 1.01);
    // vertexes[i].z *= animCounter % 122 < 62 ? 1.01 : (1 / 1.01);
  }
}
...
void* xDrawLoop(void* in) {
  ...
  while(1) {
    ...
    if(image == NULL) {
      ...
      for(int i = 0; i < SCREEN_WIDTH; i++) {
        for(int j = 0; j < SCREEN_HEIGHT; j++) {
          XPutPixel(image, SCREEN_WIDTH - (i + 1), SCREEN_HEIGHT - (j + 1), drawPixels[i][j]);
        }
      }
